...
export function ChatInterface() {
  ...
  const scrollToBottom = () => {
    // Force scroll to messages, not input
  ...
  useEffect(() => {
    scrollToBottom();
    // Remove focus from input after scrolling
  ...
  return (
    <motion.div 
      ...
      className="flex flex-col h-[450px] bg-white rounded-3xl shadow-2xl border border-gray-200 overflow-hidden"
    >
      {/* Header */}
      ...
      <div className="border-t border-gray-200 p-5 bg-white">
        <form onSubmit={(e) => {
          e.preventDefault();
          handleSubmit(e);
          // Blur input after submit to keep focus on messages
          setTimeout(() => {
            if (inputRef.current) {
              inputRef.current.blur();
            }
